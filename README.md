...
帯状・円形・深度マップ画像・全体の4モード。分散色、ボケの縁、ハイライト、玉ボケ強調、縦横比、ピント位置を調整できます。

「玉ボケ強調」は明るい点を滑らかな円形ボケとして持ち上げます。0%では元のCoolBlurと同じ計算になり、50〜100%が実用的な目安です。

全体を玉ボケにする場合は、Web Testの「全体ぼけ」プリセット、またはYMM4で `ピントの形=全体`、`画質=高品質（512）`、`半径=40px`、`分散量=0%`、`ハイライト=90%`、`玉ボケ強調=100%`、`縦横比=1`、`リニアライト=ON` を指定してください。
...
- 深度マップの画素対応とフォーカス量は元版に合わせていますが、AE CPU版のFFTぼかしはDirect2Dでは使用しないため、深度モードのぼけ形状は完全一致の対象外です。
