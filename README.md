...
添付画像を中央クロップし、896×504の正確な16:9にした入力を使っています。比較画像はAI生成ではなく、元プラグインGPU版の計算順（前処理、ハイブリッドVogel点群、色分散、ボケ縁、線形ライト、Auto時の縮小→拡大合成）をCPU参照コードで再現したものです。AE／YMM4の実アプリ画面キャプチャではないため、画素完全一致の証拠としては扱いません。
