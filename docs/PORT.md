...
- 追加の「玉ボケ強調」は0%で元実装と同一。使用時だけ選択画質のVogel点群で明部層を滑らかにし、スクリーン合成する
...
- 元GPU版と同じAuto downsample（32px以上で2倍、96px以上で4倍）。前処理済み画像をcontent-onlyでbox平均し、低解像度でギャザー、線形補間で拡大。実効半径4pxまでの遷移ランプでフル解像度原画と合成
...
- 深度は静止画ファイルのみ。サイズ違い画像の画素対応とフォーカス量はAE CPU版に合わせているが、ぼかし本体はDirect2Dギャザーであり、AE CPU版が半径8px以上で使うFFT経路とは一致しない。別レイヤー、動画、OpenEXR深度の取り込みは未対応。
...
- 本番HLSLをD3D11 WARPで描画し41ケースを検証。全画質・ボケの縁・正負の分散、半透明の一定色、半径0、ピント内、完全透明、深度未指定／焦点一致／サイズ違い、端の透明化、色の正規化、点光源の分散を確認。Auto downsampleの縮小と4px遷移合成も本番シェーダーを実描画して数値比較する。
